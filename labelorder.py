...
def convert_annotation(image_id):
    ...
    out_file = open('../image/%s.txt' % (image_id), 'w')
...
for image_set in sets:
    if not os.path.exists('C:\\yolov4\\darknet\\build\\darknet\\x64\\data\\ImagePath\\'):
        os.makedirs('../ImagePath\\')
    image_ids = open('../ImageSets/%s.txt' % (image_set)).read().strip().split()
    list_file = open('../ImagePath/%s.txt' % (image_set), 'w')
    for image_id in image_ids:
        list_file.write('C:\\yolov4\\darknet\\build\\darknet\\x64\\data\\image\\%s.jpg\n' % (image_id))
